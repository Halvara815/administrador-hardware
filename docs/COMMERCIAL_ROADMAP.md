...
Estado: **NEEDS_USER_VERIFICATION** (2026-09-05: Telemetría térmica mediante arquitectura desacoplada `ThermalSensorProvider` con frontera de sesión determinista por escaneo. Implementados `WmiThermalZoneProvider` [dK -> °C y degradación de permisos], `StorageThermalProvider` [reutiliza fiabilidad SMART F2], `NvidiaGpuThermalProvider` [vía binario oficial en rutas protegidas y banderas oficiales de throttling térmico] y `NullThermalProvider` de rollback. Reset de sesión implementado para `ThermalSnapshotProvider` y `SafePowerShellRunner` sin datos obsoletos entre escaneos consecutivos. AMD/Intel GPU, voltajes y ventiladores de placa no expuestos nativamente se declaran `NOT_SUPPORTED`. 237 passed, 29 subtests passed [229 passed, 8 skipped en entornos sin display], ruff limpio, mypy sin errores en 36 archivos. Pendiente validación física en hardware heterogéneo).
